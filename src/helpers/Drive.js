...
import { leerArchivoXlsx } from "../utils/XlsxFiles";
...
export default class DriveProvider {
    ...
    #crearEstructuraArchivos() {
        const resCarpeta = this.#crearArchivo(DRIVE_FOLDER_NAME, true);
        if (!resCarpeta.success) {
            return { success: false, error: resCarpeta.error };
        }
        const resArchivo = this.#crearArchivo(DRIVE_FILENAME, false, resCarpeta.data.id);
    ...
    #verificarEstructuraArchivos() {
        const resCarpeta = this.#verificarExistenciaArchivo(DRIVE_FOLDER_NAME, true);
        if (!resCarpeta.success) {
            return { success: false, error: resCarpeta.error };
        }
        const resArchivo = this.#verificarExistenciaArchivo(DRIVE_FILENAME, false);
        if (!resArchivo.success) {
            return { success: false, error: resArchivo.error };
        }
        this.#idArchivo = resArchivo.data.id;
    ...
    crearRespaldoDiagnosticos(token, nombreArchivo,)
};